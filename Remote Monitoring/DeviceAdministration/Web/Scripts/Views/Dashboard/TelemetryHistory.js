            ...
            if (lastData) {
                if (lastData.length > 0) {
                    for (var i = 0; i < lastData.length; i++) {
                        if (lastData[i].deviceTelemetryModels[0]) {
                            var status = 0;
                            if (lastData[i].deviceTelemetryModels[0].length == 1) {
                                status = lastData[i].deviceTelemetryModels[0].values['parkStatus'];
                            }
                            else {
                                status = lastData[i].deviceTelemetryModels[lastData[i].deviceTelemetryModels.length - 1].values['parkStatus'];
                            }

                            if (status == 1) {
                                var eleId = lastData[i].deviceId;
                                $("#" + eleId).show();
                            }
                            else {
                                var eleId = lastData[i].deviceId;
                        ...
                        var messenger = lastData[i].messengerUser;
                        if (messenger != "")
                        {
                            $("#img" + lastData[i].deviceId).show();
                            $("#user" + lastData[i].deviceId).text(messenger);
                        }
                        else
                        {
                            $("#img" + lastData[i].deviceId).hide();
                            $("#user" + lastData[i].deviceId).text('');
                        }
                    }
                }
